...
if (!APPROVED_API_KEY) {
	console.log('APPROVED_API_KEY env variable not found, api key will not be required.');
} else {
	console.log('APPROVED_API_KEY env variable found, api key will be required for create, update, delete.');
}
...
function expressMiddleware(req, res, next) {
	console.log('expressMiddleware before checkAuthorizationKey, req.url: '+ req.url);
	checkAuthorizationKey(req, function(isAuthorized, msg){
		if (!isAuthorized) {
			console.log('checkAuthorizationKey returning 401: isAuthorized='+ isAuthorized +': '+ msg);
			return res.json({msg:msg}, 401);
		}
		console.log('expressMiddleware after checkAuthorizationKey, req.url: '+ req.url);
		next();
	});
};
